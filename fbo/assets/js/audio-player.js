(function () {
  ...
  try {
    globalAudio.setAttribute("controlsList", "nodownload");
  } catch (e) {}
    ...
    playlist.forEach((track, idx) => {
      ...
      const progress = hasDuration
        ? Math.max(
            ...
            Math.min(
              1000,
              Math.round((globalAudio.currentTime / duration) * 1000),
            ),
          )
        : 0;
  ...
  function clearReactiveBars() {
    ...
    document
      .querySelectorAll(".audio-player.reactive-enabled")
      .forEach((el) => {
  ...
  function setTrack(idx) {
    ...
    document
      .querySelectorAll(".audio-player.playing")
  ...
  async function loadMetadata(src) {
    try {
      ...
      if (meta.image && meta.image.data) {
        const blob = new Blob([meta.image.data], {
          type: meta.image.mime || "image/jpeg",
        });
  ...
  async function enhanceAudioItems() {
    ...
    for (const item of items) {
      ...
      if (inGrid) {
        ...
        indicator.innerHTML =
          '<svg width="36px" height="36px" viewBox="0 0 24 24" stroke-width="1" fill="none" xmlns="http://www.w3.org/2000/svg" color="#ffffff"><path d="M4 13.4998L3.51493 13.6211C2.62459 13.8437 2 14.6437 2 15.5614V17.4383C2 18.356 2.62459 19.156 3.51493 19.3786L5.25448 19.8135C5.63317 19.9081 6 19.6217 6 19.2314V13.7683C6 13.378 5.63317 13.0916 5.25448 13.1862L4 13.4998ZM4 13.4998V13C4 8.02944 7.58172 4 12 4C16.4183 4 20 8.02944 20 13V13.5M20 13.5L20.4851 13.6211C21.3754 13.8437 22 14.6437 22 15.5614V17.4383C22 18.356 21.3754 19.156 20.4851 19.3786L18.7455 19.8135C18.3668 19.9081 18 19.6217 18 19.2314V13.7683C18 13.378 18.3668 13.0916 18.7455 13.1862L20 13.5Z" stroke="#ffffff" stroke-width="1" stroke-linecap="round" stroke-linejoin="round"></path></svg>';
      ...
      const startPlayback = () => {
        const targetSrc =
          playlist[idx] && playlist[idx].src ? playlist[idx].src : "";
        let isSame = false;
        try {
          if (globalAudio.src) {
      ...
      if (!inGrid) {
        ...
        if (trackRef.controls) {
          ...
          const startSeeking = () => {
            trackRef.controls.isSeeking = true;
          };
          const stopSeeking = () => {
          ...
          trackRef.controls.range.addEventListener("touchstart", startSeeking, {
            passive: true,
          });
          trackRef.controls.range.addEventListener("touchend", stopSeeking, {
            passive: true,
          });
          ...
          trackRef.controls.range.addEventListener("input", () => {
            ...
            const next =
              (Number(trackRef.controls.range.value) / 1000) * duration;
    ...
    document.querySelectorAll(".list-video-overlay").forEach((overlay) => {
      ...
      const maybeEnableLongVideoControls = () => {
        ...
        const syncVideoControls = () => {
          ...
          videoControlsState.button.textContent = video.paused
            ? "Play"
            : "Pause";
          ...
          if (!videoControlsState.isSeeking) {
            ...
            videoControlsState.current.textContent = formatTime(
              video.currentTime,
            );
          }

          videoControlsState.total.textContent =
        ...
        const startSeeking = () => {
          videoControlsState.isSeeking = true;
        };
        const stopSeeking = () => {
        ...
        progress.addEventListener("touchstart", startSeeking, {
          passive: true,
        });
  ...
  // Watch for dynamically added preview items (e.g. upload/recorded items)
  // and re-run enhanceAudioItems() when new .item elements or audio elements appear.
  try {
    let pendingEnhance = 0;
    const scheduleEnhance = () => {
      if (pendingEnhance) return;
      pendingEnhance = window.setTimeout(() => {
        pendingEnhance = 0;
        try {
          enhanceAudioItems();
        } catch (e) {
      ...
      for (const m of mutations) {
        if (m.addedNodes && m.addedNodes.length) {
          for (const n of m.addedNodes) {
            ...
            if (
              el.querySelector &&
              (el.querySelector("audio") || el.querySelector(".audio-player"))
            ) {
              scheduleEnhance();
              return;
            }
