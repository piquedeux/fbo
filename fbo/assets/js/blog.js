...
(() => {
  ...
  form.addEventListener("submit", (event) => {
    ...
    const stepOne = window.confirm(
      "Switch to compose mode and delete media manually first. Continue anyway?",
    );
    if (!stepOne) {
      const deniedConfirm = window.confirm(
        "You declined the compose/media cleanup warning. Continue to final danger confirmation anyway?",
      );
      if (!deniedConfirm) {
        event.preventDefault();
        return;
    ...
    const stepTwo = window.confirm(
      "Really delete blog and all data? This cannot be undone nor restored.",
    );
    if (!stepTwo) {
      event.preventDefault();
      return;
