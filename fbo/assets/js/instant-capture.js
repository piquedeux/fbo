(function () {
  ...
  const ready = () => {
    ...
    const form =
      document.getElementById("inlineUploadForm") || input?.closest("form");
    ...
    if (
      document.getElementById("instantRecordBtn") ||
      document.getElementById("instantPhotoBtn") ||
      document.getElementById("instantVideoBtn")
    ) {
      return;
    }
    ...
    const ensureLocationInputs = () => {
      let coordsInput = form.querySelector(
        'input[name="instant_location_coords"]',
      );
      let labelInput = form.querySelector(
        'input[name="instant_location_label"]',
      );
      if (!coordsInput) {
    ...
    recordBtn.addEventListener("click", async () => {
      ...
      mediaRecorder.addEventListener("stop", () => {
        const blobType = recorderMime.includes("mp4")
          ? "audio/mp4"
          : recorderMime;
    ...
    const openCaptureModal = async (mode) => {
      ...
      locationInput.className =
        "upload-auth-input instant-capture-location-input";
      ...
      const stopBtn =
        mode === "video" ? document.createElement("button") : null;
      if (stopBtn) {
        stopBtn.type = "button";
      ...
      cameraSelect.className =
        "upload-auth-input instant-capture-camera-select";
      ...
      const refreshCameraControls = async () => {
        ...
        try {
          ...
          const hasFrontCamera = videoInputs.some((d) =>
            frontPattern.test(String(d.label || "")),
          );
          const hasRearCamera = videoInputs.some((d) =>
            rearPattern.test(String(d.label || "")),
          );
          const canFlipFacingMode =
            hasMultipleCameras &&
            isCoarsePointer &&
            hasFrontCamera &&
            hasRearCamera;
      ...
      const capturePhoto = () => {
        ...
        canvas.toBlob(
          (blob) => {
            if (!blob) return;
            previewFile = new File([blob], "photo_" + Date.now() + ".jpg", {
              type: "image/jpeg",
            });
      ...
      const startVideoRecording = () => {
        ...
        recorder.addEventListener("stop", () => {
          stopRecordTimers();
          let type =
            recorder && recorder.mimeType ? recorder.mimeType : "video/mp4";
          // In some environments the recorder mimeType may report audio/ for webm with video
          // Force a video/ mime when we are in video mode and the mime incorrectly reports audio/
          if (mode === "video" && type.startsWith("audio/")) {
            type = type.replace(/^audio\//, "video/");
          }
          ...
          // Ensure the File uses a video/ mime and matching extension so server classifies correctly
          previewFile = new File([blob], "video_" + Date.now() + "." + ext, {
            type,
          });
